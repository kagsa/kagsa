...
def __init__(value):
    parse_string = Paths.getFile('parse_string.py', 'r').read().replace(
    ...
    parser = Paths.getFile('parser.py', 'r').read().replace(
        'from .parse_string import __init__ as parse_string', '')
    exec(parser, space)
    lex = Lexer(value)
    parse = space['main'](lex)
    parse = '\n'.join(parse)
    parse = parse.replace('STR_SYM_TWO', '\\"').replace(
        'STR_SYM_ONE', "\\''").replace('STR_SYM_THREE', "``")
